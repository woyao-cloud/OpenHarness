"""Hook execution engine — simplified for mini_src."""
...
from mini_src.core.messages import ConversationMessage
...
from mini_src.hooks.types import AggregatedHookResult, HookResult
...
class HookExecutor:
    ...
    def __init__(self, registry: dict[str, list[HookDefinition]], context: HookExecutionContext) -> None:
    ...
    async def execute(self, event: HookEvent, payload: dict[str, Any]) -> AggregatedHookResult:
        ...
        for hook in self._registry.get(event.value, []):
